...
Help is a full-width `CliGrid` document: its text, indentation, blank rows, and trailing row fill
render with the selected theme's default text ink over the selected theme's default background, and
semantic roles colour the keys on top of that surface. The grid wraps descriptions at the measured
destination width while preserving their structural indentation, so output does not depend on
terminal auto-wrap. `--theme light` therefore stays readable on a dark terminal. See
