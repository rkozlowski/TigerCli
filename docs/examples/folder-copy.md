...
Its identity, description, and shared `0.9.3` version live in the project metadata and are imported with `UseAssemblyMetadata(...)`, the preferred pattern for a normal executable app (see [command apps → app metadata](../guides/command-apps.md#app-metadata)). That shape is useful for operation tools where the app's main behavior is obvious from the executable name:
